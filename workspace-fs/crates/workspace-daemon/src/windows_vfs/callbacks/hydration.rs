    ...
    guard("get_placeholder_info", || {
        ...
        let (is_directory, size, content_hash) = match runtime.file(&path) {
            Some(file) => (
                ...
                Some(
                    file.content_id()
                        .map_err(|error| callback_error("hash resident file", &error))?,
                ),
            ),
        ...
        let content = content_hash.unwrap_or_else(|| *blake3::hash(path.as_bytes()).as_bytes());
        placeholder.VersionInfo.ProviderID[..16].copy_from_slice(&[0x52; 16]);
        placeholder.VersionInfo.ContentID[..32].copy_from_slice(&content);
        unsafe {
...
pub unsafe extern "system" fn get_file_data(
    ...
    guard("get_file_data", || {
        ...
        .map_err(|error| callback_error("read resident file", &error))?
    })
}
